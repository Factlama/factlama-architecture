...
This is the centralized component specification for the separate `factlama-reliability` code repository. Start with [CURRENT_IMPLEMENTATION.md](CURRENT_IMPLEMENTATION.md) for the checked prototype status and [docs/implementation.md](docs/implementation.md) for the REL task ledger. The short README in the code repo points here.
...
`schemas/`, `core/`, `judges/`, `tests/` and `examples/` hold a migrated prototype (claim extraction, evidence mapping, scoring, policy, four judge/provider implementations, 116 passing tests) carried over from a pre-pivot single-repo build. It is a head start, not a finished implementation of any REL task — [docs/PROTOTYPE_MIGRATION_NOTES.md](docs/PROTOTYPE_MIGRATION_NOTES.md) lists exactly what it gets right for free and what must change before it satisfies `CONTRACTS.md` and ADR-004/006/010–012/015.
