...
Status tracking has one source per level: [ROADMAP.md](ROADMAP.md) owns EPIC-01–14 plus EPIC-04b; the architecture repo's [Reliability task ledger](factlama-reliability/docs/implementation.md) owns REL-01–15 and [Observability task ledger](factlama-observability/docs/implementation.md) owns OBS-01–16. [EXECUTION_PLAN.md](EXECUTION_PLAN.md) orders work and defines cross-repository gates without duplicating task status. Implementation repos retain only short README and agent entrypoints; update these central ledgers after code and acceptance checks pass.
